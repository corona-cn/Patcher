...
package patcher.core.impl
import patcher.core.binder.*
import patcher.core.native.*
...
value class Process(val memorySegment: MemorySegment) {
    ...
    companion object {
        ...
        fun getAllProcesses(): List<Process> {
            ...
            val snapshot = Kernel32.kernel32.CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, OFFSET_DW_SIZE)

            try {
                val firstEntry = NativeFunctionBinder.sharedArena.allocate(PROCESS_ENTRY_SIZE_LONG).apply {
                    set(ValueLayout.JAVA_INT, OFFSET_DW_SIZE, PROCESS_ENTRY_SIZE_INT)
                }

                if (Kernel32.kernel32.Process32FirstW(snapshot, firstEntry) != 0) {
                    processes.add(Process(firstEntry))
                    do {
                        val nextEntry = NativeFunctionBinder.sharedArena.allocate(PROCESS_ENTRY_SIZE_LONG).apply {
                            set(ValueLayout.JAVA_INT, OFFSET_DW_SIZE, PROCESS_ENTRY_SIZE_INT)
                        }
                        processes.add(Process(nextEntry))
                    } while (Kernel32.kernel32.Process32NextW(snapshot, nextEntry) != 0)
                }
            } finally {
                Kernel32.kernel32.CloseHandle(snapshot)
            }
        ...
        fun openHandle(pid: Long, desiredAccess: Int = PROCESS_ALL_ACCESS): MemorySegment {
            return Kernel32.kernel32.OpenProcess(desiredAccess, 0, pid)
        }
        ...
        fun closeHandle(handle: MemorySegment): Boolean {
            return (Kernel32.kernel32.CloseHandle(handle)) != 0
        }
    }
