...
value class MemoryPatcher(private val processHandle: MemorySegment) {
    ...
    fun read(address: Long, size: Int): ByteArray {
        ...
        val buffer = NativeFunctionBinder.sharedArena.allocate(size.toLong())
        val bytesRead = NativeFunctionBinder.sharedArena.allocate(ValueLayout.JAVA_INT)
        val result = Kernel32.kernel32.ReadProcessMemory(
            processHandle,
    ...
    fun write(address: Long, data: ByteArray): Boolean {
        ...
        val buffer = NativeFunctionBinder.sharedArena.allocate(data.size.toLong())
        buffer.copyFrom(MemorySegment.ofArray(data))

        val bytesWritten = NativeFunctionBinder.sharedArena.allocate(ValueLayout.JAVA_INT)
        val result = Kernel32.kernel32.WriteProcessMemory(
            processHandle,
            MemorySegment.ofAddress(address),
            buffer,
            data.size.toLong(),
